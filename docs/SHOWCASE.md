...
- "Every 15 seconds" in 1/7 and in the blurb is the **local** cadence. The Vercel deployment ticks every 60 seconds, driven by a cron job rather than a timer, and the dashboard's NEXT TICK countdown reports whichever is actually in force. If the demo footage is recorded from the deployment, say 60 seconds; do not let the copy and the video disagree.
