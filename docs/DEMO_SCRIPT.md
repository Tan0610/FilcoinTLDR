...
| **Say (while it counts down)** | Locally: "Fifteen second tick. Sense, decide, act. Watch the log." From the deployment: "Sixty second tick, driven by a cron job, not by this page. Sense, decide, act. Watch the log." Read the cadence off the `NEXT TICK` cell rather than from this script — it reports the interval actually in force, so it is right in both builds. |
